...
const Login = () => {
  ...
  const handleSignupForm = () => {
    ...
    setError(null)
    setInvalidCredentialsError(null) // Clear both errors when switching forms
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setInvalidCredentialsError(err.message)
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <div className="relative bg-black/70 backdrop-blur-xl rounded-2xl p-8 shadow-2xl border border-white/20 hover:border-red-500/40 transition-all duration-300 
          ...
          <div className="relative z-10">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* Password Input - REMOVED pb-3 to fix padding */}
              <div>
                <input
                  id="password"
                  name="password"
              ...
              {/* Error Messages - FIXED syntax and positioning */}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 disabled:from-gray-600 disabled:to-gray-700 text-white font-semibold py-3.5 px-4 rounded-xl transition-all duration-200 transform hover:scale-[1.03] focus:outline-none focus:ring-2 focus:ring-red-500/50 shadow-xl disabled:cursor-not-allowed disabled:transform-none"
              >
                {isLoading ? (
